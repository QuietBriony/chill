...
for (const html of [indexHtml, sessionHtml]) {
  ...
  assert.match(html, /style\.css\?v=pwa-2/);
}
assert.match(indexHtml, /engine\.js\?v=pwa-2/);
assert.match(sessionHtml, /engine\.js\?v=pwa-2/);
assert.match(sessionHtml, /session\.js\?v=pwa-2/);

const sw = text("sw.js");
assert.match(sw, /const CACHE_PREFIX = "chill-pwa"/);
assert.match(sw, /const VERSION = `\$\{CACHE_PREFIX\}-v2`/);
for (const asset of ["style.css?v=pwa-2", "engine.js?v=pwa-2", "session.js?v=pwa-2"]) {
